...
echo "export MYSQL_PS1='\u@\h [\d]> '" | tee -a ~/.profile
source ~/.profile
...
sudo apt-get update
echo "percona-server-server-5.7 percona-server-server-5.7/root-pass password root" | sudo debconf-set-selections
echo "percona-server-server-5.7 percona-server-server-5.7/re-root-pass password root" | sudo debconf-set-selections
